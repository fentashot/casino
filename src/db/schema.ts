...
export const casinoSpin = pgTable(
  ...
  (table) => [
    ...
    index("spin_created_at_idx").on(table.createdAt),
...
/**
 * One row per finished blackjack game.
 * Persisted at the moment the game reaches phase="finished" so that
 * statistics endpoints can aggregate across both roulette and blackjack.
 */
export const blackjackRound = pgTable(
  "blackjack_round",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    /** Sum of all hand bets (including doubles / splits) */
    totalBet: numeric("total_bet").notNull(),
    /** Total amount returned to the player (wins + pushes) */
    totalWin: numeric("total_win").notNull(),
    /**
     * Snapshot of player hands results, e.g.:
     * [{ result: "blackjack", bet: 100 }, { result: "win", bet: 100 }]
     * Stored as JSONB for easy querying and to avoid a separate table.
     */
    handsSnapshot: jsonb("hands_snapshot").notNull().$type<
      Array<{
        result: string;
        bet: number;
        doubled: boolean;
    ...
    >(),
    /** Balance AFTER the game settled (for running-balance reconstruction) */
    balanceAfter: numeric("balance_after").notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [
    index("bj_round_user_id_idx").on(table.userId),
    index("bj_round_created_at_idx").on(table.createdAt),
  ],
);
...
export const blackjackRoundRelations = relations(blackjackRound, ({ one }) => ({
  user: one(user, {
    fields: [blackjackRound.userId],
    references: [user.id],
  }),
}));
