...
import { TrendingUp, Rows3 } from "lucide-react";
      ...
      <div className="flex items-start sm:items-center justify-between gap-4 animate-fade-in">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Drop the ball &middot; Choose rows &amp; difficulty &middot; Win up to 1000&times;
          </p>
        </div>

        <div className="hidden sm:flex items-center gap-2 flex-wrap justify-end">
          <StatPill icon={<TrendingUp className="h-3.5 w-3.5" />} label="Max payout" value="1000×" />
          <StatPill icon={<Rows3 className="h-3.5 w-3.5" />} label="Rows" value="8–16" />
        </div>
      </div>
