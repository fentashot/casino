...
function PlinkoBadge({ multiplier, profit }: { multiplier: string; profit: number }) {
	...
	return (
		<div
			className={cn(
				"flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[9px] font-extrabold leading-none",
				isWin
...
function MultiplierPill({ multiplier, profit }: { multiplier: string; profit: number }) {
	...
	return (
		<div className="hidden sm:flex items-center gap-1 shrink-0">
			<span
				className={cn(
					"inline-flex items-center justify-center h-5 px-1.5 rounded text-[9px] font-bold border",
					isWin
			...
			{visible.map((r, i) => (
				<span
					...
					className={cn(
						...
						RESULT_CLASS[r] ?? "bg-muted text-muted-foreground border-border",
					)}
				>
					{RESULT_LABEL[r] ?? "?"}
				</span>
