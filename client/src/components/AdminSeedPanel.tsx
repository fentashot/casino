...
import { AlertTriangle, Eye, RefreshCw, Shield } from "lucide-react";
...
import { fetchSeeds, revealSeed, rotateSeed } from "@/lib/roulette/api";
		...
		<Card>
			...
			<CardContent className="space-y-4">
				...
				<div className="flex items-center gap-4">
					...
					>
						<RefreshCw
							className={`h-4 w-4 mr-2 ${rotateMutation.isPending ? "animate-spin" : ""}`}
						/>
						Rotate Server Seed
					</Button>
					<p className="text-sm text-muted-foreground">
							...
							<TableBody>
								...
								{(!data?.seeds || data.seeds.length === 0) && (
									<TableRow>
										...
										>
											No seeds found. The first seed will be created on the
											first spin.
										</TableCell>
									</TableRow>
								)}
