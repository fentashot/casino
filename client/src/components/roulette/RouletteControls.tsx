import type { betSchema } from "@server/zodTypes";
import { Loader } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import type z from "zod";
import { BalanceDisplay } from "@/components/roulette/BalanceDisplay";
...
const CELL_BASE =
	"select-none cursor-pointer rounded text-sm font-medium text-white text-center transition-colors";
...
function useBetManager(
	chipValue: number,
	chipCount: number,
	disabled: boolean,
) {
	const [pendingStacks, setPendingStacks] = useState<
		Record<BetKey, PendingStack>
	>({});
	...
	const removeFromBasket = useCallback(
		(index: number, currentChipValue: number) => {
...
function NumberCell({ number, pendingAmount, onMouseDown }: NumberCellProps) {
	return (
		...
		>
			...
			{pendingAmount && (
				<div className="badge bottom-0">{formatValue(pendingAmount)}</div>
			)}
		</button>
	);
}
...
function OutsideBetButton({
	...
	return (
		<div className="relative">
			...
			{pendingAmount && (
				<div className={badgeClass}>{formatValue(pendingAmount)}</div>
			)}
		</div>
	);
}
	...
	return (
		<>
			{basket.map((bet, i) => (
				<div
					key={i}
					className="px-2 py-1 rounded bg-zinc-800 text-sm flex items-center gap-1"
				>
					<div className="font-mono text-xs">
						{bet.numbers?.length
							? `n=${bet.numbers.join(",")}`
							: (bet.choice ?? bet.color)}
					</div>
...
export default function RouletteControls({
	...
	const hasNoBets =
		basket.length === 0 && Object.keys(pendingStacks).length === 0;
				...
				<div className="grid grid-cols-[auto,1fr,auto] grid-rows-[auto,1fr]">
					...
					<div className="relative">
						<button
							...
							onContextMenu={(e) => e.preventDefault()}
							className={`${CELL_BASE} h-full min-h-[96px] w-12 green-cell`}
						>
							<div className="w-full h-full flex items-center justify-center">
								0
							</div>
							{getPending("straight:0") && (
								<div className="absolute bottom-2 right-2 bg-yellow-300 text-black small-font px-1 rounded-sm z-10 pointer-events-none">
									{formatValue(getPending("straight:0") || 0)}
								</div>
							)}
						</button>
					</div>
					...
					<div className="row-span-2 space-y-1 mx-1">
						...
						<div className="grid grid-cols-3 gap-1 mt-2">
							{[
								{ key: "dozen:1st12", label: "1st 12" },
								{ key: "dozen:2nd12", label: "2nd 12" },
								{ key: "dozen:3rd12", label: "3rd 12" },
							].map(({ key, label }) => (
								<OutsideBetButton
									key={key}
									label={label}
									className="rounded w-full h-10 black-cell text-white font-medium text-xs border border-zinc-600"
							...
							{[
								{
									...
									label: "1–18",
									className:
										"rounded w-full h-10 black-cell text-white font-medium text-xs",
								},
								{
									key: "even_odd:even",
									label: "Even",
								...
								},
								{
									...
									label: "Red",
									className:
										"rounded red-cell w-full h-10 text-white font-medium text-xs",
								},
								{
									key: "red_black:black",
									label: "Black",
									className:
										"rounded black-cell w-full h-10 text-white font-medium text-xs border border-zinc-600",
								},
								{
									key: "even_odd:odd",
									label: "Odd",
						...
						<div className="flex justify-center gap-1 pt-3 pb-1">
							<Button
								...
								variant="outline"
								disabled={hasNoBets}
								className={`px-4 py-2 rounded text-white w-full border-0 ${hasNoBets ? "bg-zinc-700 opacity-50 cursor-not-allowed" : "bg-zinc-600 hover:bg-zinc-500"}`}
							>
								Reset All
							</Button>
							<Button
								...
								className={`w-full px-4 py-2 rounded text-white border-0 ${!onPlaceBets || !basket.length ? "bg-zinc-700 opacity-50 cursor-not-allowed" : "green-cell hover:brightness-110"}`}
							>
								{loading ? (
									<Loader className="animate-spin" />
								) : (
									`Place Bets (${basket.length})`
								)}
							</Button>
						</div>
					</div>
          ...
          >
            {balance !== undefined ? formatBalance(balance) : 'N/A'}
				...
				<div className="flex max-w-[700px] flex-wrap gap-2">
					<BasketDisplay
						basket={basket}
						onRemove={(i) => removeFromBasket(i, chipValue)}
					/>
				</div>
			</div>
		</div>
