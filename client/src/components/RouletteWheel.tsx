...
export const AnimatedWheel: React.FC<Props> = ({
	...
	const cancelScheduledTicks = useCallback(() => {
	...
	const scheduleTick = useCallback(
		(ctx: AudioContext, atTime: number, freq: number) => {
			try {
	...
	const scheduleAllTicks = useCallback(
		(
			ctx: AudioContext,
			spinDegrees: number,
		...
		[cancelScheduledTicks, pockets, scheduleTick],
	);

	const isSpinningRef = useRef(false);

	// Spin zawsze w TYM SAMYM kierunku i ze STAŁĄ liczbą obrotów
	const spinToPocket = useCallback(
		async (targetIndex: number | null) => {
			...
			setIsSpinning(false);
			onSpinEnd?.(ROULETTE_SEQUENCE[targetIndex]);
		},
		[
			anglePerPocket,
			cancelScheduledTicks,
	...
	const spinToPocketRef = useRef(spinToPocket);
	spinToPocketRef.current = spinToPocket;

	useEffect(() => {
		if (
			typeof targetNumber === "number" &&
			targetNumber !== lastTargetRef.current
		) {
			lastTargetRef.current = targetNumber;
			const idx = ROULETTE_SEQUENCE.indexOf(targetNumber);
			if (idx !== -1) spinToPocketRef.current(idx);
		}
	}, [targetNumber]);
