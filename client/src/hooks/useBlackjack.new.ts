...
export function useBlackjack(initialBalance = 0) {
  ...
  const handleGameResponse = useCallback(
    (game: BlackjackGameState, prevShown: number, clearTable = false) => {
      ...
      if (clearTable) {
        animation.prepareForDeal(game);
      }

      animation.runAnimation(
        game,
        clearTable ? 0 : prevShown,
        () => {
    ...
    [syncBalance, animation, notify, queryClient],
  );
  ...
  const deal = useCallback(
    ...
    [isLoading, animation, handleGameResponse, notify],
  );

  const takeInsurance = useCallback(
    async (decision: InsuranceDecision) => {
      if (isLoading || animation.isAnimating) return;
      setIsLoading(true);
      ...
      try {
        ...
        const prevShown = animation.shownCountRef.current;
  ...
  const performAction = useCallback(
    async (action: BlackjackAction) => {
      if (isLoading || animation.isAnimating || !serverGame) return;
      setIsLoading(true);
    ...
    [isLoading, animation, serverGame, handleGameResponse, notify],
  );
  ...
  const restoreGame = useCallback(async () => {
    try {
      ...
      if (data.game) {
        ...
        animation.restoreDisplayState(data.game);
        syncBalance(data.game.balance);
      }
    } catch {
      /* not critical */
    }
  }, [animation, syncBalance]);
  ...
  const isPlaying =
    phase === "playing" && activeHand !== null && !animation.isAnimating;
  ...
  const insuranceAvailable =
    phase === "insurance" && !isLoading && !animation.isAnimating;
  const maxInsuranceBet = activeHand ? Math.floor(activeHand.bet / 2) : 0;

  return {
    game: animation.displayGame,
    serverGame,
    phase,
    balance,
